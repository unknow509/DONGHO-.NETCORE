...
(function (Microsoft) {
    ...
    (function (ApplicationInsights) {
        ...
        (function (_InternalMessageId) {
            ...
            _InternalMessageId[_InternalMessageId["USRACT_ModelNotInArray"] = 46] = "USRACT_ModelNotInArray";
        ...
        var Util = (function () {
            ...
            Util._getVerifiedStorageObject = function (storageType) {
                ...
                try {
                    ...
                    storage.setModel(uid, uid);
                    fail = storage.getModel(uid) != uid;
                    storage.removeModel(uid);
                    if (fail) {
                        storage = null;
                    }
                }
            ...
            Util.removeStorage = function (name) {
                ...
                if (storage !== null) {
                    try {
                        storage.removeModel(name);
                        return true;
                    }
                    catch (e) {
                        var message = new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_BrowserFailedRemovalFromLocalStorage, "Browser failed removal of local storage Model. " + Util.getExceptionName(e), { exception: Util.dump(e) });
                        ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.WARNING, message);
                    }
                }
                return false;
            ...
            Util.removeSessionStorage = function (name) {
                ...
                if (storage !== null) {
                    try {
                        ...
                        var message = new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_BrowserFailedRemovalFromSessionStorage, "Browser failed removal of session storage Model. " + Util.getExceptionName(e), { exception: Util.dump(e) });
                        ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, message);
                    }
                }
                return false;
            ...
            Serializer._serializeArray = function (sources, name) {
                ...
                if (!!sources) {
                    if (!ApplicationInsights.Util.isArray(sources)) {
                        ApplicationInsights._InternalLogging.throwInternalUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.USRACT_ModelNotInArray, "This field was specified as an array in the contract but the Model is not an array.\r\n", { name: name }));
                    }
                    else {
                        output = [];
                        for (var i = 0; i < sources.length; i++) {
                            var source = sources[i];
                            var Model = Serializer._serializeObject(source, name + "[" + i + "]");
                            output.push(Model);
                        }
                    }
                }
...
(function (AI) {
    ...
    var ContextTagKeys = (function () {
        function ContextTagKeys() {
            ...
            this.internalTelemetryModelId = "ai.internal.telemetryModelId";
        ...
        var DataLossAnalyzer = (function () {
            ...
            DataLossAnalyzer.reset = function () {
                if (DataLossAnalyzer.isEnabled()) {
                    ApplicationInsights.Util.setSessionStorage(DataLossAnalyzer.ModelS_QUEUED_KEY, "0");
                }
            };
            ...
            DataLossAnalyzer.incrementModelsQueued = function () {
                try {
                    if (DataLossAnalyzer.isEnabled()) {
                        var ModelsQueued = DataLossAnalyzer.getNumberOfLostModels();
                        ++ModelsQueued;
                        ApplicationInsights.Util.setSessionStorage(DataLossAnalyzer.ModelS_QUEUED_KEY, ModelsQueued.toString());
                    }
                }
                catch (e) { }
            };
            DataLossAnalyzer.decrementModelsQueued = function (countOfModelsSentSuccessfully) {
                try {
                    if (DataLossAnalyzer.isEnabled()) {
                        var ModelsQueued = DataLossAnalyzer.getNumberOfLostModels();
                        ModelsQueued -= countOfModelsSentSuccessfully;
                        if (ModelsQueued < 0)
                            ModelsQueued = 0;
                        ApplicationInsights.Util.setSessionStorage(DataLossAnalyzer.ModelS_QUEUED_KEY, ModelsQueued.toString());
                    }
            ...
            DataLossAnalyzer.getNumberOfLostModels = function () {
                var result = 0;
                try {
                    if (DataLossAnalyzer.isEnabled()) {
                        result = isNaN(+ApplicationInsights.Util.getSessionStorage(DataLossAnalyzer.ModelS_QUEUED_KEY)) ?
                            0 :
                            +ApplicationInsights.Util.getSessionStorage(DataLossAnalyzer.ModelS_QUEUED_KEY);
                    }
                }
                catch (e) {
                    result = 0;
            ...
            DataLossAnalyzer.ModelS_QUEUED_KEY = "AI_ModelsQueued";
        ...
        var Sender = (function () {
            ...
            Sender.prototype.send = function (envelope) {
                ...
                try {
                    ...
                    ApplicationInsights.DataLossAnalyzer.incrementModelsQueued();
                }
                catch (e) {
            ...
            Sender.prototype._xhrSender = function (payload, isAsync, countOfModelsInPayload) {
                var xhr = new XMLHttpRequest();
                xhr[ApplicationInsights.AjaxMonitor.DisabledPropertyName] = true;
                xhr.open("POST", this._config.endpointUrl(), isAsync);
                xhr.setRequestHeader("Content-type", "application/json");
                xhr.onreadystatechange = function () { return Sender._xhrReadyStateChange(xhr, payload, countOfModelsInPayload); };
            ...
            Sender._xhrReadyStateChange = function (xhr, payload, countOfModelsInPayload) {
                if (xhr.readyState === 4) {
                    if ((xhr.status < 200 || xhr.status >= 300) && xhr.status !== 0) {
                        Sender._onError(payload, xhr.responseText || xhr.response || "");
                    }
                    else {
                        Sender._onSuccess(payload, countOfModelsInPayload);
                    }
                }
            };
            ...
            Sender._onSuccess = function (payload, countOfModelsInPayload) {
                ApplicationInsights.DataLossAnalyzer.decrementModelsQueued(countOfModelsInPayload);
            };
            return Sender;
        })();
